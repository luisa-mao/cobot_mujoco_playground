...
from utils import create_student_network, get_student_inference_fn, get_video
...
def dagger_rollout(env, student_policy, teacher_policy, beta=0.5, num_envs=4, max_steps=500, key=None, wandb_run=None):
    ...
    def step_fn(carry, i):
        ...
        teacher_act, _ = teacher_policy(state.obs, teacher_key)
...
def loss_fn(student_params, normalizer_params, obs, expert_actions):
    # norm_params, policy_params = student_params
    # norm_params = jax.lax.stop_gradient(normalizer_params)
...
if __name__ == "__main__":
    ...
    teacher_jit_inference_fn = load_inference_without_env(restore_checkpoint_path, obs_dim=obs_dim, action_dim=action_dim)
    ...
    for iteration in range(NUM_DAGGER_ITERATIONS):
        ...
        if iteration % 5 == 0:
            frames, final_is_success = get_video(student_jit_inference_fn_w_params, env, num_envs=64, episode_length=250, rng_seed=iteration, render_every = render_every)
            fps = 1.0 / env.dt / render_every
            ...
            frames, final_is_success_teacher = get_video(teacher_jit_inference_fn, env, num_envs=64, episode_length=250, rng_seed=iteration, render_every = render_every)
